...
var PhysicsWorker = function(){
  ...
  var that = {  
    ...
    modifyNode:function(id, mods, paused){
      _physics.modifyNode(id, mods)

      // If the system is stopped, then only move the node.
      if (paused) {
        _physics.tock();
      }
      else{
...
onmessage = function(e){
  ...
  switch(e.data.type){
    case "modify":
      physics._paused = e.data.systemPaused;
      physics.modifyNode(e.data.id, e.data.mods, e.data.systemPaused)
      break
