  ...
  var Kernel = function(pSystem){
    ...
    var that = {
      ...
      graphChanged:function(changes){
        ...
        that.start(true) // <- is this just to kick things off in the non-worker mode? (yes)
      },
      ...
      particleModified:function(id, mods){
        ...
        if (USE_WORKER) {
          _physics.postMessage({type:"modify", id:id, mods:mods, systemPaused:_paused})
        }
        else {
          _physics.modifyNode(id, mods);
          var render = that.system.renderer;

          if(_attached !== undefined) { // The user defined renderer.
            _attached.redraw(); // redraw the node positions
          }
          that.start(false) // <- is this just to kick things off in the non-worker mode? (yes)
        }
      },
      ...
      //
      start:function(unpaused){
        if (_tickInterval !== null) return; // already running

        if (_paused && !unpaused) return; // we've been .stopped before, wait for unpause (should not be in start?)
